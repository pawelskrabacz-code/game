...
/**
 * Find 1-stop connecting itineraries between two airports when no
 * direct flight exists. Uses AP_IDX to find candidate layover airports
 * (reachable from origin AND able to reach destination) without scanning
 * the full ROUTES table, then validates each leg through getFlightOptions
 * so a connection is only ever offered if both legs are real, bookable
 * routes already in the data.
 *
 * Picks the cheapest economy-fare option per leg, sums both legs into a
 * single itinerary, and returns the cheapest itineraries first.
 *
 * @param {string} fromIata
 * @param {string} toIata
 * @param {number} maxResults - cap on itineraries returned (default 5)
 * @returns {Array<{via, leg1, leg2, totalEcoFare, totalBizFare, totalDistMi}>}
 */
function getConnectingFlights(fromIata, toIata, maxResults = 5) {
  const fromIdxs = AP_IDX[fromIata] || [];
  const toIdxs   = AP_IDX[toIata]   || [];

  const fromNeighbors = new Set();
  for (const i of fromIdxs) {
    const r = ROUTES[i];
    fromNeighbors.add(r[_O] === fromIata ? r[_D] : r[_O]);
  }
  const toNeighbors = new Set();
  for (const i of toIdxs) {
    const r = ROUTES[i];
    toNeighbors.add(r[_O] === toIata ? r[_D] : r[_O]);
  }

  const candidates = [...fromNeighbors].filter(ap =>
    toNeighbors.has(ap) && ap !== fromIata && ap !== toIata
  );

  const itineraries = [];
  for (const via of candidates) {
    const leg1Options = getFlightOptions(fromIata, via);
    const leg2Options = getFlightOptions(via, toIata);
    if (!leg1Options.length || !leg2Options.length) continue;
    const leg1 = leg1Options.reduce((a, b) => (b.ecoFare < a.ecoFare ? b : a));
    const leg2 = leg2Options.reduce((a, b) => (b.ecoFare < a.ecoFare ? b : a));
    itineraries.push({
      via,
      leg1, leg2,
      totalEcoFare: leg1.ecoFare + leg2.ecoFare,
      totalBizFare: leg1.bizFare + leg2.bizFare,
      totalDistMi:  leg1.distMi  + leg2.distMi,
    });
  }

  itineraries.sort((a, b) => a.totalEcoFare - b.totalEcoFare);
...
if (typeof module !== 'undefined') {
  module.exports = {
    ...
    checkRouteUnlockEligibility, getFlightOptions, getConnectingFlights, buildFeasibilityDoc, routeObj
  };
}
